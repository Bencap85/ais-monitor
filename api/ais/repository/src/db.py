...
def batch_upsert_ships(conn: connection, ship_batch: List) -> None:
    try:
        with conn.cursor() as cur:
            ...
            conn.commit()
            logger.debug(f"Successfully inserted/updated {len(ship_batch)} records into database")
        
    except Exception as e:
        logger.error(f"Failed to commit db update: {e}")
        conn.rollback()
...
def batch_update_history(conn: connection, ship_batch: List) -> None:
    try:
        with conn.cursor() as cur:
            ...
            execute_values(cur, query, values, template="(%s, %s, %s, %s, ST_SetSRID(ST_MakePoint(%s, %s), 4326))")
            conn.commit()
            logger.debug(f"Successfully inserted/updated {len(ship_batch)} HISTORY records into database")
...
def batch_upsert_static_data(conn: connection, static_batch: List) -> None:
    try:
        with conn.cursor() as cur:
            ...
            conn.commit()
            logger.debug(f"Successfully inserted/updated {len(static_batch)} static ship records")
